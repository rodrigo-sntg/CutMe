...
type SQSConsumer struct {
	svc      repository2.QueueClient
	s3Client repository2.StorageClient
	queueURL string
	handler  entity.SQSMessageHandler
}

func NewSQSConsumer(svc repository2.QueueClient, s3Client repository2.StorageClient, queueURL string, handler entity.SQSMessageHandler) *SQSConsumer {
	return &SQSConsumer{
		svc:      svc,
		s3Client: s3Client,
		queueURL: queueURL,
